...
async function loadData(from, to) {
  const sessions = await sql`SELECT session_id, anonymous_user_id, app_version, started_at, last_active_at, device_type FROM sessions WHERE started_at >= ${from}::timestamptz AND started_at < (${to}::date + INTERVAL '1 day') ORDER BY started_at DESC`;
  const queries = await sql`SELECT id, session_id, app_version, queried_at, target_date, regions, region_count, result_success, api_elapsed_ms, api_error FROM weather_queries WHERE queried_at >= ${from}::timestamptz AND queried_at < (${to}::date + INTERVAL '1 day') ORDER BY queried_at DESC`;
  const events = await sql`SELECT id, session_id, app_version, event_name, created_at, metadata FROM events WHERE created_at >= ${from}::timestamptz AND created_at < (${to}::date + INTERVAL '1 day') ORDER BY created_at DESC`;
  ...
  try {
    ...
    if (action === "csv") {
      ...
      if (table === "sessions") rows = data.sessions;
        ...
        rows = [
          { kpi: "multi_region_rate", numerator: kpi.multiRegion.numerator, denominator: kpi.multiRegion.denominator, rate: kpi.multiRegion.rate },
          { kpi: "chart_open_rate", numerator: kpi.chartOpen.numerator, denominator: kpi.chartOpen.denominator, rate: kpi.chartOpen.rate },
          { kpi: "date_change_rate", numerator: kpi.dateChange.numerator, denominator: kpi.dateChange.denominator, rate: kpi.dateChange.rate },
        ];
    ...
    return json({ kpis: calculateKpis(data), sessions: buildSessionRows(data).slice(0, 200) });
