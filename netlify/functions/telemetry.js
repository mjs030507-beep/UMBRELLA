  ...
  try {
    ...
    if (body.type === "weather_query_start") {
      ...
      await sql.begin(async (tx) => {
        await tx`INSERT INTO weather_queries (id, anonymous_user_id, session_id, app_version, queried_at, target_date, regions, region_count) VALUES (${queryId}, ${cleanText(body.anonymous_user_id, 100)}, ${cleanText(body.session_id, 100)}, ${cleanText(body.app_version, 30)}, ${body.occurred_at || new Date().toISOString()}, ${body.target_date}, ${JSON.stringify(regions)}::jsonb, ${regionCount}) ON CONFLICT (id) DO NOTHING`;
        await tx`INSERT INTO events (anonymous_user_id, session_id, app_version, event_name, created_at, metadata) VALUES (${cleanText(body.anonymous_user_id, 100)}, ${cleanText(body.session_id, 100)}, ${cleanText(body.app_version, 30)}, 'weather_search', ${body.occurred_at || new Date().toISOString()}, ${JSON.stringify({ target_date: body.target_date, region_count: regionCount })}::jsonb)`;
      });
    ...
    if (body.type === "weather_query_result") {
      ...
      await sql.begin(async (tx) => {
        await tx`UPDATE weather_queries SET result_success = ${success}, api_elapsed_ms = ${elapsed}, api_error = ${error} WHERE id = ${queryId} AND session_id = ${cleanText(body.session_id, 100)}`;
        await tx`INSERT INTO events (anonymous_user_id, session_id, app_version, event_name, created_at, metadata) VALUES (${cleanText(body.anonymous_user_id, 100)}, ${cleanText(body.session_id, 100)}, ${cleanText(body.app_version, 30)}, ${eventName}, ${body.occurred_at || new Date().toISOString()}, ${JSON.stringify(safeMetadata(body.metadata))}::jsonb)`;
      });
      return json({ ok: true }, 201);
    }
